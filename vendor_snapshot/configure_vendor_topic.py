"""
configure_vendor_topic.py — Stage 9.3 Step 3.

ONE-TIME instance update. Adds a custom "VENDOR_FACTS" memory topic to our
existing Agent Engine instance, alongside the default managed topics (kept,
not replaced, so USER_PREFERENCES etc. still work). Includes one few-shot
example since Google's docs recommend this for custom topics.

Note: imports from vertexai._genai.types (not vertexai.types as some docs
show) — confirmed against installed google-cloud-aiplatform==1.158.0.
"""
...
def main():
    ...
    print("Instance updated with VENDOR_FACTS custom topic.")
    print(updated)
